// pages/index.js
// Sales Comp Week '26 â Registration Tracker dashboard.

import { useEffect, useState } from "react";

// Everstage brand palette â extracted from everstage.com CSS (2026 refresh)
// Dark nav: rgb(65,25,45)=#41192d Â· CTA button: rgb(248,150,80)=#f89650
// Teal (logo): #48c39e Â· Body bg: #fafaf7 Â· Text on dark: #fafaf7
const C = {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "ui-sans-serif, system-ui, -apple-system, sans-serif", padding: "32px 24px" }}>
      <div style={{ maxWidth: 1360, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16, marginBottom: 32 }}>
          <div>
            ...
            <h1 style={{ margin: 0, fontSize: 28, fontWeight: 800, letterSpacing: "-0.5px", color: C.text }}>
              Sales Comp Week '26 â Registration Tracker
            </h1>
            <p style={{ margin: "6px 0 0", color: C.sub, fontSize: 14 }}>
              Live channel-level registrations Â· Sessions: Jun 23 Â· Jul 7 Â· Jul 9
            </p>
          </div>
        ...
        {loading && !data && <p style={{ color: C.sub }}>Loading live numbers from HubSpotâ¦</p>}
        ...
        {data && (
          <>
            ...
            {/* ââ Filtered submissions callout ââ */}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#1e1008", border: `1px solid #5a3810`, borderRadius: 10, padding: "12px 16px", marginBottom: 20, fontSize: 13 }}>
                ...
                <div>
                  ...
                  <span style={{ color: C.sub, marginLeft: 10 }}>
                    ...
                    {data.filteredStats.everstage > 0 && data.filteredStats.noEmail > 0 && "  Â·  "}
                    {data.filteredStats.noEmail > 0 && `${data.filteredStats.noEmail} no-email / no-record contacts`}
                  </span>
                  <span style={{ color: "#7a6050", marginLeft: 10 }}>
                    ({data.filteredStats.rawTotal} raw â {data.total} counted)
                  </span>
                </div>
              </div>
            )}

            {/* ââ Week-over-Week table ââ */}
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "auto", marginBottom: 24 }}>
              <div style={{ padding: "16px 20px 0", borderBottom: `1px solid ${C.border}` }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: C.sub, letterSpacing: "0.08em", textTransform: "uppercase" }}>
                  Channel breakdown â Week over Week
                </span>
              </div>
                      ...
                      <td key={ci} style={{ ...td, fontWeight: 800, color: C.text, borderLeft: ci === 0 ? "none" : `1px solid ${C.border}` }}>
                        ...
                        {data.wowPct[ci] != null && (
                          <div style={{ fontSize: 11, fontWeight: 600, color: data.wowPct[ci] >= 0 ? C.up : C.down, marginTop: 2 }}>
                            {data.wowPct[ci] >= 0 ? "â²" : "â¼"} {Math.abs(data.wowPct[ci])}%
                          </div>
                        )}
                      </td>
                    ))}
            ...
            {/* ââ Daily registration detail ââ */}
                ...
                <div style={{ overflowX: "auto" }}>
                  <div style={{ display: "flex", minWidth: "max-content", padding: "16px 20px", gap: 12 }}>
                    {data.weekDailyBreakdown.map((days, wi) => {
                      ...
                      return (
                        <div key={wi} style={{ minWidth: 110, background: "#200e18", border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden" }}>
                          {/* Week header */}
                          <div style={{ padding: "8px 10px", background: "#2e1428", borderBottom: `1px solid ${C.border}` }}>
                            ...
                            <div style={{ fontSize: 12, fontWeight: 700, color: C.accent, marginTop: 3 }}>{weekTotal} regs</div>
                          </div>
                          {/* Day rows */}
                          {days.map((day) => (
                  ...
                  return (
                    <div key={s.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "18px 20px", borderTop: `3px solid ${past ? C.up : C.accent}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                        <span style={{ color: C.accent, fontWeight: 800, fontSize: 12, letterSpacing: "0.05em" }}>SESSION {i + 1} Â· {s.id}</span>
